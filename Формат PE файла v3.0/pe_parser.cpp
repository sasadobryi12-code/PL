...
constexpr uint32_t IMAGE_ORDINAL_FLAG32 = 0x80000000u;
...
private:
    ...
    const char* importNameFromRva(uint32_t rva) const {
        const uint32_t off = rvaToFileOffset(rva);
        if (off == 0 || off + sizeof(IMAGE_IMPORT_BY_NAME) >= size()) return nullptr;
        const char* name = reinterpret_cast<const char*>(base() + off + sizeof(uint16_t));
        for (size_t i = off + sizeof(uint16_t); i < size(); ++i) {
            if (base()[i] == '\0') return name;
        }
        return nullptr;
    }

    void printImportFunctions(uint32_t lookupRva) const {
        if (lookupRva == 0) return;

        for (uint32_t idx = 0;; ++idx) {
            if (is64_) {
                const uint32_t entryRva = lookupRva + idx * sizeof(uint64_t);
                const uint32_t entryOff = rvaToFileOffset(entryRva);
                if (entryOff == 0 || entryOff + sizeof(uint64_t) > size()) break;

                const uint64_t thunk =
                    *reinterpret_cast<const uint64_t*>(base() + entryOff);
                if (thunk == 0) break;

                if (thunk & IMAGE_ORDINAL_FLAG64) {
                    const uint16_t ordinal = static_cast<uint16_t>(thunk & 0xFFFF);
                    std::cout << "    #" << ordinal << std::endl;
                } else {
                    const char* fn = importNameFromRva(static_cast<uint32_t>(thunk));
                    if (fn) std::cout << "    " << fn << std::endl;
                }
            } else {
                const uint32_t entryRva = lookupRva + idx * sizeof(uint32_t);
                const uint32_t entryOff = rvaToFileOffset(entryRva);
                if (entryOff == 0 || entryOff + sizeof(uint32_t) > size()) break;
                ...
                if (thunk & IMAGE_ORDINAL_FLAG32) {
                    ...
                    const char* fn = importNameFromRva(thunk);
                    if (fn) std::cout << "    " << fn << std::endl;
                }
            }
        }
    ...
    void printImports() const {
        ...
        for (;;) {
            ...
            if (dllName) {
                ...
                const uint32_t lookupRva = desc->OriginalFirstThunk != 0
                                               ? desc->OriginalFirstThunk
                                               : desc->FirstThunk;
                printImportFunctions(lookupRva);
                any = true;
            }
