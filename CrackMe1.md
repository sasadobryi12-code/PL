...
### 1. Какой корневой раздел реестра открывает программа?
Программа обращается к разделу HKEY_CURRENT_USER.

### 2. Какой путь (подключ) она ищет?
Внутри этого раздела программа проверяет путь Software\uLCnSBOdTo.

### 3. Имя какого параметра ей нужно?
Программа считывает параметр с именем 3ClgZ.

### 4. Что произойдёт, если этот подключ или параметр не найден?
Функция RegQueryValueExA вернёт код ошибки, после чего выполнение перейдёт к подпрограмме sub_4017BB, которая вызывает ExitProcess(0). Пользователь при этом увидит, как на мгновение появляется и сразу исчезает окно консоли.

### 5. Какое значение параметра заставит программу вывести FLAG?
В параметре должна храниться строка: cknloKtidY12eClAi0ZkyfPkSwUTPvH3sNLELaWm.

### 6. Почему RegQueryValueExA вызывается дважды?
Первый вызов нужен, чтобы определить размер данных (длину флага), хранящихся в реестре. После этого программа выделяет буфер, размер которого в два раза больше полученной длины (чтобы данные точно поместились). Второй вызов уже копирует само значение флага из реестра в этот буфер.
